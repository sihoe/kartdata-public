/* sykkelturer-app.js — baseline (samme logikk som fungerende monolitt) */
(function () {
  "use strict";

  const CFG = (window.SYKKELTURER_APP_CONFIG && typeof window.SYKKELTURER_APP_CONFIG === "object")
    ? window.SYKKELTURER_APP_CONFIG
    : {};

  const root = document.getElementById("sykkelturer-app");
  if (!root) return;
  ...
  const DATA_ROUTES_URL = CFG.DATA_ROUTES_URL || "https://cdn.jsdelivr.net/gh/sihoe/kartdata-public@main/routes503.json";
  const DATA_EXTRAS_URL = CFG.DATA_EXTRAS_URL || "https://cdn.jsdelivr.net/gh/sihoe/kartdata-public@main/routes_extras_legacy_clean502.json";
  const DATA_TEXTS_URL  = CFG.DATA_TEXTS_URL  || "https://cdn.jsdelivr.net/gh/sihoe/kartdata-public@main/routes_texts_and_highlights502.json";
  ...
  const MAP_DIV_ID      = CFG.MAP_DIV_ID   || "map";
  ...
  function normalizeRouteType(v) {
    ...
    if (["out_and_back","outandback","out-back","outback","frem_og_tilbake","fremogtilbake"].includes(s)) return "out_and_back";
    if (["point_to_point","pointtopoint","a_to_b","atob","one_way","oneway","en_vei","envei"].includes(s)) return "point_to_point";
    return s;
  }
  ...
  function typeLabel(type, lang) {
    ...
    return map[type]?.[lang] || map[type]?.no || type;
  }

  function difficultyIconUrl(diff){ return `${SYMBOLS_BASE}symbols-biking-${diff}.svg`; }
  function symbolUrl(type){ return `${SYMBOLS_BASE}symbols-${type}.svg`; }
  function routeColorByDifficulty(diff){ return DIFF_COL[diff] || "#37394E"; }
  ...
  function surfaceParts(route){
    const cat = route?.surface?.categoryKm || {};
  ...
  function loadRouteToMap(route){
    return new Promise((resolve) => {
      ...
      gpx.on("loaded", (e) => {
        ...
        if (!poly){ resolve(false); updateVisibleList(); return; }
      ...
      gpx.on("error", () => { resolve(false); updateVisibleList(); });
    });
  }

  // ====== data normalize + merge (som i monolitt)
  ...
  function durationMedian(extras){
    const d = extras?.durationEstimate?.display;
    if (!d) return "";
  ...
  function merge(routesRaw, extrasRaw, textsRaw){
    const routes = normalizeObj(routesRaw);
    const extras = normalizeObj(extrasRaw);
    const texts  = normalizeObj(textsRaw);

    const merged = [];

    for (const [id, r402] of Object.entries(routes)){
      const ex = extras[id] || {};
      const tx = texts[id] || {};
      ...
      const nameNo = asText(r402.name, "no")
